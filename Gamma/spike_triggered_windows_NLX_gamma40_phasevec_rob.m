function [eeg_epochs_grouped, sPhase, sPhase2] = spike_triggered_windows_NLX_gamma40_phasevec_rob(eeg_ind_spikes, EEG, f1, f2, Fs)
...
for k = 1:N
    ...
    eeg_epoch_to_insert = EEG(eeg_ind_spikes(k) - 400: eeg_ind_spikes(k) + 400);
        ...
        if (max(eeg_ind_spikes(k) + 400) > length(EEG))
            eeg_epoch_to_insert = EEG(eeg_ind_spikes(k) - 400: length(EEG));
        end
    end
